...
@SearchTags({"attribute swap", "weapon swap", "lunge swap", "mace swap",
	...
	implements PlayerAttacksEntityListener, UpdateListener
{
	private final EnumSetting<Mode> mode = new EnumSetting<>("Mode",
		"Simple: swaps to a fixed hotbar slot on attack.\n"
			...
			+ "Smart also supports lunge swapping (spear slot).",
	...
	private final SliderSetting spearSlot = new SliderSetting("Spear slot",
		"Hotbar slot containing spear/mace (Smart lunge swapping).", 3, 1, 9, 1,
		ValueDisplay.INTEGER);
	...
	private final CheckboxSetting lungeSwapping = new CheckboxSetting(
		"Lunge swapping",
		"Immediately switches to spear slot on click to reduce lunge cooldown (Smart mode).",
		true);
	...
	public void onUpdate()
	{
		...
		// detect exact click (consume once per press)
		if(MC.options.keyAttack.consumeClick()
			&& mode.getSelected() == Mode.SMART && lungeSwapping.isChecked())
		{
			// Immediately switch to spear slot
			doSwap(spearSlot.getValueI() - 1);
			
			// DO NOT swing here — the original click is the attack
		}
	}
	...
	private int getSmartSlot(Entity target)
	{
		...
		// lunge swapping handled in onUpdate
		return -1;
	}
