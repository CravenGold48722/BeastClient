...
public final class TrajectoriesHack extends Hack implements RenderListener
{
	...
	private Trajectory getTrajectory(float partialTicks)
	{
		...
		// Build the path with adaptive sub-steps for ≤1/4-block accuracy.
		// Drag and gravity are scaled per sub-step so physics stays continuous.
		...
		{
			double speed = arrowMotion.length();
			if(speed < 1e-9)
				break;
				
			// Step at most 0.25 blocks so the curve stays within 1/4 block
			// of the real parabola at all initial speeds.
			double scale = Math.min(1.0, 0.25 / speed);
			Vec3 nextPos = arrowPos.add(arrowMotion.scale(scale));
			
			// Check for block collision on this sub-step.
			...
			// Check for entity collision on this sub-step.
			AABB box = new AABB(arrowPos, nextPos);
			EntityHitResult eResult = ProjectileUtil.getEntityHitResult(player,
				arrowPos, nextPos, box, predicate, maxDistSq);
			...
			// Drag first then gravity, scaled to this sub-step's tick fraction.
			// Math.pow(0.99, scale) gives the exact fractional per-tick drag.
			arrowMotion = arrowMotion.scale(Math.pow(0.99, scale)).add(0,
				-gravity * scale, 0);
			
			if(arrowPos.y < MC.level.getMinY())
				break;
		}
